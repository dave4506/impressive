...
class Chatbar extends React.Component {
  ...
  render() {
    ...
    const {placeholder,status={indicator:"",text:""}} = this.props;
    return (
      <div className={`${s["chatbar"]} ${s["chatbar-standard"]}`}>
        ...
        <input value={value} onChange={(e)=>{this.setState({value:e.target.value})}} placeholder={placeholder} type="text" className={`${s["chatbar-input"]} ${s["chatbar-standard-input"]}`}/>
        <div onClick={this.onClick.bind(this)} className={`${s["chatbar-send"]}`}>
